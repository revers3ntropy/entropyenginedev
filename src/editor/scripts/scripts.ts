...
export async function loadScripts () {
	...
	for (let scriptPath of scriptPaths) {
		let scriptRaw = await (await fetch(`${scriptPath}?${window.genCacheBust()}`)).text();

		const name = window.nameFromScriptURL(scriptPath);
...
export async function reloadScriptsOnEntities () {
	...
	for (let scriptPath of scriptPaths) {
		...
		let name = window.nameFromScriptURL(scriptPath);
		...
		scriptPrimitives[name] = new ESNamespace(new ESString(name), env.getSymbolTableAsDict());
	}

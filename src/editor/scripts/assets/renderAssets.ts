...
import {cullString} from 'entropy-engine/src/util/general.js';
...
export const renderAssets = async div => {
	const projectAssets = await request('/get-assets', apiToken);
	let assetsHTML = '';
	...
	const projectSize = await request('/folder-size', apiToken, {

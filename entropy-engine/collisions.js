...
import { JSONifyComponent } from "./util.js";
...
export class CircleCollider extends Collider {
    ...
    overlapsPoint(transform, point) {
        return point.distTo(transform.position.clone
...
export class RectCollider extends Collider {
    ...
    tick() { }
    overlapsPoint(transform, point) {
        return point.isInRect(transform.position.clone
...
function getNewVelocityFromCollision(V1, V2, m1, m2, p1, p2, contactAngle) {
    ...
    return [
        new v2(v1x_prime, v1y_prime),
        new v2(v2x_prime, v2y_prime)
    ];
}
// collisions types
function circleCircleCollision(sprite1, sprite2, onCollision) {
    ...
    [body1.velocity, body2.velocity] = getNewVelocityFromCollision(body1.velocity, body2.velocity, body1.mass, body2.mass, transform1.position, transform2.position, phi);
...
function circleRectCollision(circle, rect, onCollision) {
    ...
    const rectD = new v2(colliderRect.width, colliderRect.height).mul(rect.transform.scale);
    ...
    if (!circle.Static)
        transformCircle.position.add(moveDirection.clone
            .normalise()
            .scale(overlap / (rect.Static ? 1 : 2)));
    if (!rect.Static)
        transformRect.position.sub(moveDirection.clone
            .normalise()
...
function rectRectCollision(sprite1, sprite2, onCollision) {
    ...
    const pos2 = transform2.position
        .clone
        .add(collider2.offset);
    const dimensions1 = new v2(collider1.width, collider1.height).mul(sprite1.transform.scale);
    const dimensions2 = new v2(collider2.width, collider2.height).mul(sprite2.transform.scale);
    ...
    // resolve collision
    if (!sprite1.Static)
        transform1.position.add(overlap.clone.scale(-1 / (sprite2.Static ? 1 : 2)));
    if (!sprite2.Static)
        transform2.position.add(overlap.clone.scale(1 / (sprite1.Static ? 1 : 2)));
    if (!sprite1.hasComponent('Body') || !sprite2.hasComponent('Body')) {
        // finish early without doing the dynamic collision
        onCollision();
        return;
    ...
    const bounce1 = body1.material.bounciness;
    const bounce2 = body2.material.bounciness;
    if (collisionLeftRight) {
...
function collide(sprite1, sprite2, scriptCollide) {
...
export function collideAll(sprites, scriptCollide) {
